...
class DPVO:
    ...
    def terminate(self):
        ...
        for i in range(self.n):
            ...
            #print("self.pg.tstamps_[i]:",self.pg.tstamps_[i],"---",self.traj[self.pg.tstamps_[i]])
    ...
    def motion_probe(self):
        ...
        with autocast(enabled=self.cfg.MIXED_PRECISION):
            ...
            ctx = self.imap[:,kk % (self.M * self.pmem)] #(1,DIM)
            net, (delta, weight, _) = \
                self.network.update(net, ctx, corr, None, ii, jj, kk)
        #print("delta.shape ",delta.shape)  [1,96,2]
        #print("torch.quantile(delta.norm(dim=-1).float(), 0.5)=",torch.quantile(delta.norm(dim=-1).float(), 0.5))
    ...
    def __call__(self, tstamp, image, intrinsics):
        ...
        if self.n > 0 and not self.is_initialized:
            if self.motion_probe() < 2.0:       #原来是<2.0
